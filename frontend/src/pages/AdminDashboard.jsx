...
const AdminDashboard = () => {
  ...
  // ── Canviar estat comanda (Testeo) ───────────────────────────────────────
  const handleCanviarEstatComanda = async (id, newEstat) => {
    const res = await apiFetch(`${API}/api/dashboard/admin/comandes/${id}/estat`, {
      method: 'PATCH',
      body: JSON.stringify({ estat: newEstat }),
    });
    if (!res) return;
    if (res.ok) {
      setActionMsg(`✅ Estat de la comanda actualitzat a "${newEstat}"`);
      loadData();
    } else {
      const e = await res.json();
      setActionMsg(`❌ ${e.message}`);
        ...
        <div className="db-admin-card" style={{
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['ID', 'Client', 'Data', 'Articles', 'Total', 'Estat', 'Accions (Test)'].map(h => (
                    <th key={h} style={{
                      color: '#6b7280', fontSize: '0.75rem', fontWeight: 700,
                      textTransform: 'uppercase', letterSpacing: '0.08em',
                      padding: '0.75rem 1rem', textAlign: 'left',
                  ...
                  <tr key={c._id} className="action-row" style={{
                    ...
                    <td style={{ padding: '1rem' }}>
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        {c.estat === 'pending' && (
                          <button
                            onClick={() => handleCanviarEstatComanda(c._id, 'paid')}
                            title="Marcar com a Pagat (TEST)"
                            style={{
                              padding: '0.35rem 0.75rem', borderRadius: '0.4rem', border: 'none',
